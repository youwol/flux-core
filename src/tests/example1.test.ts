...
*/
const pack = new FluxPack(AUTO_GENERATED)
...
export namespace SimpleModule {
    ...
    export class PersistentData {
        ...
        constructor({ operationType }: { operationType?: Operations } = {}) {
             ...
             */
            this.operationType =
                operationType != undefined ? operationType : Operations.ADDITION
        }
    }
     ...
     */
    const straightNumber = expect_<number>({
    ...
    export class Module extends ModuleFlux {
        ...
        constructor(params) {
            ...
            this.addInput({
                ...
                contract: expectCount<number>({
                    count: 2,
                    when: permissiveNumber,
                }),
        ...
        do(
            ...
            const result = operationsFactory[configuration.operationType](data)
    ...
    export function renderHtmlElement(mdle: Module): HTMLElement {
        return render({
             ...
             */
            innerText: attr$(mdle.result$, ({ data }) => {
...
test('valid inputs', (done) => {
    const branches = ['--|~dataEmittor~|---|~myModule~|--']
    ...
    const graph = parseGraph({ branches, modules })
    ...
    modules.myModule.result$.pipe(take(1)).subscribe(({ data }) => {
        // Test scenario 1

        expect(data).toBe(15)
        const text = document.getElementById('my-module-label-id').innerText
        expect(text).toBe('The last computed result is: 15')
         ...
         */
        modules.dataEmittor.emit({
            data: [5, 6],
            configuration: {
                operationType: SimpleModule.Operations.MULTIPLICATION,
            },
        })
    })
    ...
    modules.myModule.result$.pipe(take(1)).subscribe(({ data }) => {
        ...
        expect(data).toBe(30)
        const text = document.getElementById('my-module-label-id').innerText
        expect(text).toBe('The last computed result is: 30')
    ...
    modules.myModule.result$.pipe(take(1)).subscribe(({ data }) => {
        ...
        expect(data).toBe(20)
        const text = document.getElementById('my-module-label-id').innerText
        expect(text).toBe('The last computed result is: 20')
        done()
    })
})
...
test('invalid configuration', (done) => {
    ...
    modules.dataEmittor.emit({
        data: [5, 10],
        configuration: { operationType: 'division' },
    })
