 ...
 *
 * @format
 * @Schema({ pack: MapReduce, description: "Persistent Data of MapReduce" }) export class PersistentData {
 * @Flux({ pack: packCore, namespace: MapReduce, id: "MapReduce", displayName: "MapReduce", description: "A map reduce plugin on groups", compatibility: { Component: { condition: (mdle) => mdle instanceof GroupModules.Module, description: "A map reduce plugin should be associated to a group module" }, Straight: { condition: (mdle:ModuleFlux) => mdle.inputSlots.length==1 &&  mdle.outputSlots.length==1, description: "A map reduce plugin should be associated to a group module with one input and one output"
 * @BuilderView({ namespace: MapReduce, icon: icon }) export class Module extends PluginFlux<GroupModules.Module> {
 */
