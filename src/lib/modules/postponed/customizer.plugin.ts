 ...
 *
 * @format
 * @Property({ description: "definition", type:"code"}) readonly code: string
 * @Property({ description: "output count", type:"integer"}) readonly outputCount: number
 * @Flux({ pack:           packCore, namespace:      Customizer, id:             "Customizer", displayName:    "Customizer", description:    "Customizer", compatibility: { ModuleView: { condition : (mdle) => true, description : "A Customizer plugin should be plug to a module with view"
 * @BuilderView({ namespace:      Customizer, icon:           svgIcon }) export class Module extends PluginFlux<Component.Module>  implements SideEffects{
 */
