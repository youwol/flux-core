 ...
 *
 * @format
 * @Schema({ pack: packCore, description: "Persistent Data of ComponentReplica" }) export class PersistentData {
 * @Property({ description:"Generate the id of the replica", type: 'code' }) replicaIdGenerator: string
 * @Property({ description:"name of the attribut that conveys tab data context" }) innerContextName: string
 * @Flux({ pack: packCore, namespace: ComponentReplica, id: "ComponentReplica", displayName: "ComponentReplica", description: "Replicator of group or component", compatibility: { Component: { condition: (mdle) => mdle instanceof GroupModules.Module, description: "A tabs should be associated to a component or a group"
 * @BuilderView({ namespace: ComponentReplica, icon: icon }) export class Module extends ReplicatorModuleBase<ReplicaData> {
 */
