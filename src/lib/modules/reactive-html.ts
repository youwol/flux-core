...
export function createHTMLElement({
        ...
        const sub = data
            .pipe(
                tap(() => {
                    ...
                    _.remove(subscriptions, (e, i) =>
                        childrenSubscriptions.includes(e),
                    )
            ...
            .subscribe((data) => {
                ...
                const i0 = subscriptions.length
                const newDiv = createHTMLElement({
                    data,
                    subscriptions,
                    namespace,
                    classesDict,
    ...
    const elem =
        namespace == 'svg' || data.namespace == 'svg'
            ? document.createElementNS('http://www.w3.org/2000/svg', data.tag)
    ...
    const specialBindings = {
        tag: () => {},
        attributes: (elem, values) =>
            Object.entries(values).forEach(([k, v]) =>
                elem.setAttribute(k, String(v)),
            ),
        style: (elem, values) =>
            Object.entries(values).forEach(([k, v]) =>
                elem.style.setProperty(k, String(v)),
            ),
        ...
        onmouseenter: (elem: HTMLDivElement, value) =>
            (elem.onmouseenter = value),
        onmouseleave: (elem: HTMLDivElement, value) =>
            (elem.onmouseleave = value),
        ...
        _class: (elem, value) => {
            if (elem['previousDynamicClasses']) {
                elem['previousDynamicClasses'].forEach((c) =>
            ...
            const classes = value
                .split(' ')
                .reduce((acc, e) => acc.concat(classesDict[e].split(' ')), [])
    ...
    entries
        .filter(([k, v]) => k.substring(0, 2) != '__')
        .forEach(([k, v]) => {
            if (v instanceof Observable) {
                const sub = v.subscribe((val) => {
