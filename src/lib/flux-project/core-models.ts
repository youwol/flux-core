...
export class DescriptionBox {
    constructor(
        public readonly descriptionBoxId: string,
...
export class BuilderRendering {
    constructor(
        public readonly modulesView: Array<ModuleView>,
...
export class Requirements {
    constructor(
        public readonly fluxComponents: Array<string>,
        public readonly fluxPacks: Array<string>,
        public readonly libraries: Object,
        public readonly loadingGraph: any,
    ) {}
}
...
export class LayerTree {
    ...
    getLayerRecursive(testFct: (LayerTree) => boolean): LayerTree {
        ...
        return this.children
            .map((c) => c.getLayerRecursive(testFct))
            .filter((d) => d)[0]
    }

    getLayersRecursive(testFct: (LayerTree) => boolean): LayerTree[] {
        const children = this.children
            .map((c) => c.getLayersRecursive(testFct))
            .flat()
        return testFct(this)
            ? new Array<LayerTree>(this).concat(children)
            : children
    }

    getChildrenModules() {
        return this.moduleIds.concat(
            ...this.children.map((l) => l.getChildrenModules()),
        )
    }

    getChildrenLayers() {
        return this.children.concat(
            ...this.children.map((l) => l.getChildrenLayers()),
        )
    }
    ...
    getHTML({ recursive }: { recursive: boolean } = { recursive: false }) {
        ...
        const layersComponents = this.getLayersRecursive(
            (layer) => layer.html != '' && layer.layerId != this.layerId,
        ).reduce(
            (acc, e) => ({
                ...acc,
                ...{ ['Component_' + e.layerId]: toHtml(e.html as any) },
            }),
            {},
        )
        const root = toHtml(this.html as any)
        if (!root) {
            return root
        }
        while (Object.entries(layersComponents).length > 0) {
            const previousLength = Object.entries(layersComponents).length
    ...
    getCSS(
        ...
        return this.getLayersRecursive(() => true).reduce(
            (acc, e) => acc + '\n' + e.css,
            '',
        )
    }
...
export function getCollectionsDelta<T>(
    oldCollection: Array<T>,
    newCollection: Array<T>,
): { createdElements: Array<T>; removedElements: Array<T> } {
    const createdElements = newCollection.filter(
        (x) => !oldCollection.includes(x),
    )
    const removedElements = oldCollection.filter(
        (x) => !newCollection.includes(x),
    )
...
export interface WorkflowDelta {
    connections: {
        createdElements: Array<Connection>
        removedElements: Array<Connection>
    }
    modules: {
...
export function workflowDelta(oldWf: Workflow, newWf: Workflow): WorkflowDelta {
    const diffsConnection = { createdElements: [], removedElements: [] }
    const diffModules = { createdElements: [], removedElements: [] }
    if (newWf.connections !== oldWf.connections) {
        const diffs = getCollectionsDelta(oldWf.connections, newWf.connections)
    ...
    if (newWf.modules !== oldWf.modules) {
        const diffs = getCollectionsDelta(oldWf.modules, newWf.modules)
        diffModules.createdElements.push(...diffs.createdElements)
        diffModules.removedElements.push(...diffs.removedElements)

        const createdMdlesId = diffs.createdElements.map((m) => m.moduleId)
        const deletedMdlesId = diffs.removedElements.map((m) => m.moduleId)

        diffsConnection.createdElements.push(
            ...newWf.connections.filter((c) =>
                createdMdlesId.includes(c.end.moduleId),
            ),
            ...newWf.connections.filter((c) =>
                createdMdlesId.includes(c.start.moduleId),
            ),
        )

        diffsConnection.removedElements.push(
            ...oldWf.connections.filter(
                (c) =>
                    deletedMdlesId.includes(c.start.moduleId) ||
                    deletedMdlesId.includes(c.end.moduleId),
            ),
        )
    }
    if (newWf.plugins !== oldWf.plugins) {
        const diffs = getCollectionsDelta(oldWf.plugins, newWf.plugins)
        diffModules.createdElements.push(...diffs.createdElements)
        diffModules.removedElements.push(...diffs.removedElements)
    ...
    diffsConnection.createdElements = Array.from(
        new Set(diffsConnection.createdElements),
    )
    diffsConnection.removedElements = Array.from(
        new Set(diffsConnection.removedElements),
    )

    diffModules.createdElements = Array.from(
        new Set(diffModules.createdElements),
    )
    diffModules.removedElements = Array.from(
        new Set(diffModules.removedElements),
    )

    const count =
        diffsConnection.createdElements.length +
        diffsConnection.removedElements.length +
        diffModules.createdElements.length +
        diffModules.removedElements.length
    return {
