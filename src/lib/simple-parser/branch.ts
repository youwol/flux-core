...
export class Branch{
    ...
    constructor( ... steps : Array<Step>){
        ...
        this.ends        = steps.slice(1).map(  step => new ConnectionEnd(step.module,step.inputSlot, step.adaptor)) 
        this.observers   = steps.map(  (step:any) => step.externalObservers ).filter( d => d!=undefined) 
