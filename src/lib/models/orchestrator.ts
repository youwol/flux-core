...
export class StaticStorage{
    ...
    has<T>(type:string, id:string) : boolean {
        ...
        return this.of<T>(type).get(id) ? true : (this.parentStorage ? this.parentStorage.has<T>(type,id) : false)
    }
    ...
    listener<T>(type:string, id:string) : Observable<T>{
        ...
        let subjects = this.__get(type,id).filter( d=>d)
        return merge(...subjects)  as Observable<T>
    }
    __get(type, id){
        return  [this.of(type).get(id)].concat(this.parentStorage ? this.parentStorage.__get(type,id) : [])
    }
}
...
export class Orchestrator extends ModuleFlux {
    ...
    constructor(params){
        ...
        this.staticStorage = params.staticStorage ||  StaticStorage.defaultStorage
    ...
    static get$<T2>(typeName, staticStorage : StaticStorage, id: string): Observable<T2>{
