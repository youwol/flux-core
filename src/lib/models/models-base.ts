 ...
 *
 * @format
 * @Flux({ id: 'simulator', pack, namespace: SimulatorModule })
 * @BuilderView({ icon: '<text> Simulator </text>', namespace: SimulatorModule }) export class Module extends ModuleFlux {
 * @Schema() class PersistentData{
 * @property({ description: 'control the threshold of the simulation' }) threshold: number
...
export abstract class ModuleFlux {
    ...
    */
    ...
    getSlot(slotId: string): Slot | undefined {
        return [...this.inputSlots, ...this.outputSlots].find(
    ...
    log(message, data) {
        this.environment.console &&
            this.environment.console.log(this.Factory.id, message, data, this)
    }
    ...
    addOutput<T>({ id }: { id: string } = { id: 'output' }): Pipe<T> {
        ...
        this.outputSlots.push(
            new OutputSlot<{ data: T; context: Context }>(
                id,
                this.moduleId,
                {},
                piped as any,
    ...
    private processInput(
        processDataFct,
        {
            ...
            message,
            slotId,
        }: { connection: Connection; message: any; slotId: string },
    ) {
        const inputSlot = this.inputSlots.find((slot) => slot.slotId == slotId)
        const f = processDataFct.bind(this)
        ...
        context.info(`start processing function of module ${this.moduleId}`, {
            connection,
            'raw input': message,
            slotId,
        })
        ...
        if (connection && connection.adaptor) {
            adaptedInput = context.withChild(
                'execute adaptor',
                (ctx: Context) => {
                    return connection.adaptor.mappingFunction(
                        message,
                        this.helpers,
                    )
                },
        ...
        if (adaptedInput.configuration) {
            conf = context.withChild('merge configuration', (ctx: Context) => {
                const status = mergeConfiguration(
                    conf,
        ...
        const contract = inputSlot.contract

        const resolution = context.withChild('resolve contract', (ctx) => {
            let resolution = undefined
            try {
                resolution = contract.resolve(adaptedInput.data, ctx)
            } catch (error) {
                const mdleError = new ModuleError(
                    this,
            ...
            if (!resolution.succeeded) {
                throw new ContractUnfulfilledError(
                    this,
                    `The contract of the input "${slotId}" has not been Fulfilled.`,
                    resolution,
                )
            }
            return resolution
        ...
        context.info('Input provided to the module', {
            data: input.data,
            configuration: conf,
        })
        context.withChild("module's processing", (ctx) => {
            try {
                return f(
                    { ...input, ...{ context: ctx } },
                    { cache: this.cache },
                ...
                const mdleError = new ModuleError(this, error.message)
                mdleError.stack = error.stack
                throw mdleError
            }
        })
...
export abstract class ModuleRendererBuild {
    ...
    constructor({ svgIcon, Factory }: { svgIcon: string; Factory: Factory }) {
        ...
        if (!ModuleRendererBuild.iconsFactory[Factory.uid]) {
            ModuleRendererBuild.iconsFactory[Factory.uid] = {
                icon: svgIcon,
                iconNormalized: undefined,
            }
        }
    }
    ...
    icon(): { content: string; transforms: string } {
        const iconsMdl = ModuleRendererBuild.iconsFactory[this.Factory.uid]
        ...
        const transforms = getTransforms(g)
        svg.remove()
        iconsMdl.iconNormalized = {
            content: `<g style='transform:${transforms}'> ${iconsMdl.icon} </g>`,
...
export abstract class ModuleRendererRun<T extends ModuleFlux> {
    ...
    constructor(
        ...
        public readonly wrapperDivAttributes: (module: ModuleFlux) => {
            class?: string
...
export function implementsWorkflowDependantTrait(
    mdle: unknown,
): mdle is WorkflowDependantTrait {
...
export function instanceOfSideEffects(object: any): object is SideEffects {
    const obj = object as SideEffects
...
export class Adaptor {
    public readonly mappingFunction: (
        Message,
        helpers: { [key: string]: any },
    ) => Message
    ...
    constructor(
        ...
        mappingFunction:
            | string
            | ((Message, helpers: { [key: string]: any }) => Message),
    ) {
        this.mappingFunction =
            typeof mappingFunction == 'string'
...
export class ModuleError extends Error {
     ...
     */
    constructor(public readonly module: ModuleFlux, ...params) {
        super(...params)
        // @ts-ignore
