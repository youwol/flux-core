...
export function getTransforms(g) {
    ...
    return `translate(${-parentBRect.width / 4}px,${
...
export function createPlug(
    ...
    const y = vMargin - height / 2 + i * (height / (count + 1))
    const idSuff = plug.slotId + '_' + plug.moduleId
    const idPlug = `${type}-line_${idSuff}`
    const idSlot = `${type}-slot_${idSuff}`
    const base = {
        [idPlug]: createPlugLine(
            idPlug,
            `${type} plug ${plug.moduleId}`,
            (sgn * width) / 2,
            y,
            (sgn * width) / 2 + sgn * plugLength,
            y,
        ),
        [idSlot]: createPlugCircle(
            idSlot,
            plug.slotId,
            plug.moduleId,
            `${type} slot ${plug.moduleId}`,
            (sgn * width) / 2 + sgn * plugLength,
            y,
        ),
    }
    if (type == 'input') {
        return Object.assign({}, base, {
            ['arrow-' + idPlug]: {
                tag: 'path',
                class: 'input mdle-color-fill',
                attributes: {
                    d: `M${-width / 2 - 10},${y - 7} l10,7 l-10,7 z`,
                },
            },
        })
    }
...
export function genericModulePlotData({
    module,
    icon,
    width,
    vMargin,
    ...
    const headerActions =
        FluxExtensionAPIs.get('BuilderView').getHeaderActions(module)
    ...
    const plugLength = 20
    const count = Math.max(1, inputs.length, outputs.length)
    const height = 2 * vMargin + (count - 1) * vStep
    const actionWidth = headerHeight - 5
    const actionsWidth =
        Object.entries(actions).length * (actionWidth + 5) - actionWidth / 2
    width = width + actionsWidth
    const vStepInput = 50

    const toPlug = (type, plug, i) =>
        createPlug(type, plug, i, vMargin, height, count, plugLength, width)

    const actionsItems = {
        tag: 'g',
        style: {
            transform: `translateX(${width / 2 - actionsWidth}px) translateY(${
                -height / 2
            }px)`,
        },
        children: Object.entries(actions).reduce((acc, [key, val], i) => {
            const patched = Object.assign({}, val)
            patched['onclick'] = (e) => val['onclick'](e, module)
            const action = {
                tag: 'g',
                style: {
                    transform: `translateX(${i * actionWidth}px) translateY(${
                        headerHeight / 2
                    }px)`,
                },
                ...
                children: {
                    [key + '-bg']: {
                        ...
                        onclick: (e) => val['onclick'](e, module),
                    },
                    [key + '-icon']: {
                        tag: 'g',
                        style: {
                            transform: `translateX(-5.5px) translateY(-6.5px) scale(0.3)`,
                        },
                        children: { content: patched },
                    },
                },
            }
    ...
    return {
        ...
        children: {
            ...
            headerBg: {
                tag: 'path',
                class: 'module mdle-color-fill header',
                attributes: {
                    d: `M${-width / 2},${-height / 2 + headerHeight} v${-(
                        headerHeight - 10
                    )} q0,-10 10,-10 h${width - 20} q10,0 10,10  v${
                        headerHeight - 10
                    } z`,
                },
            },
            ...
            title: {
                ...
                textContent: module.configuration.title,
                attributes: {
                    x: -width / 2 + 10,
                    y: -height / 2 + headerHeight / 2 + 5,
                },
            },
...
export function genericModulePlot({
    ...
    const data = genericModulePlotData({
        ...
        vMargin,
        vStep,
        inputs: module.inputSlots,
        outputs: module.outputSlots,
        actions: {},
    })
