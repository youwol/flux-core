...
export function Flux({
    ...
    return (target) => {
        ...
        ModuleNamespace['Configuration'] = class Configuration extends (
            ModuleConfiguration
        ) {
            /**
             * should be :
...
export function BuilderView({
    ...
    return (target) => {
        ...
        if (icon != undefined) {
            ModuleNamespace['BuilderView'] = class BuilderView extends (
                ModuleRendererBuild
            ) {
                constructor() {
        ...
        if (icon != undefined && render) {
            ModuleNamespace['BuilderView'] = class BuilderView extends (
                ...
                render(mdle: ModuleFlux) {
                    const renderElem = render(mdle, this.icon().content)
                    return renderElem
                }
            }
        }
 ...
 * @returns decorator
 */
export function RenderView<T extends ModuleFlux>({
    namespace,
    wrapperDivAttributes,
    render,
}: {
    namespace: any
    wrapperDivAttributes?: (module: T) => {
        class?: string
    ...
    return (target) => {
        ...
        ModuleNamespace['RenderView'] = class RenderView extends (
            ModuleRendererRun
        )<T> {
            static get wrapperDivAttributes(): (ModuleFlux) => {
                class?: string
                style?: { [key: string]: string }
            } {
            ...
            constructor(
                module: T,
                wrapperDivAttributes: (module: ModuleFlux) => {
                    class: string
                    style: { [key: string]: string }
                },
            ) {
...
export function Schema({
    ...
    return (target) => {
        const baseClass = target.__proto__.name
        const attributes = tmpAttributes[target.name]
            ? tmpAttributes[target.name]['attributes']
            : []
...
export function Property(metadata: {
    ...
    return (target, key) => {
        ...
        if (object == undefined) {
            ...
            tmpBases[target.constructor.name] =
                target.constructor.__proto__.name
        }
...
export function flattenSchemaWithValue(instance) {
    const schema = instance.__proto__.constructor.fluxSchema
    const data = instance
    function flattenRecursive(
        ...
        const acc = { [suffix.slice(1)]: [schema, data] }

        if (schema && schema.extends) {
            const props = schema.extends
        ...
        if (schema && schema.type && schema.class && schema.class.fluxSchema) {
            const props = flattenRecursive(
                schema.class.fluxSchema,
                data,
                suffix,
            )
            _.merge(acc, props)
        ...
        if (schema && schema.attributes) {
            const props = Object.entries(schema.attributes).reduce(
                (acc: Object, [key, val]: [any, any]) => {
                    return _.merge(
                        acc,
                        flattenRecursive(val, data[key], `${suffix}.${key}`),
                    )
                },
                {},
            )
