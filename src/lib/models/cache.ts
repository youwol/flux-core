...
export class ReferenceKey implements CacheKey {
    ...
    isSame(key: ReferenceKey): boolean {
        ...
        const equalities = this.objects.map((v, i) => v == key.objects[i])
...
export class Cache {
     ...
     */
    getOrCreateWithStatus<TKey, TValue>(
        key: CacheKey,
        creatorFct: (context?: Context) => TValue,
        context?: Context,
    ): [TValue, boolean] {
        ...
        if (cached) {
            context &&
                context.info(
                    `Cache -> ${key.name}: Value retrieved from cache`,
                    cached[1],
                )
            return [cached[1], true]
        }
        const value = context
            ? context.withChild(`Cache -> ${key.name}: Value creation`, (ctx) =>
                  creatorFct(ctx),
              )
            : creatorFct()
