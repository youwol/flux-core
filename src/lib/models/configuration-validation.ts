...
export class InconsistentConfiguration<T> extends ConfigurationStatus<T> {
    ...
    constructor(
        ...
        public readonly typeErrors: Array<{
...
export function findIntrus(prefix, object, reference): Array<string> {
    const firstLayerIntrus = Object.entries(object)
        .filter(([k, v]) => reference[k] == undefined)
        .map(([k, v]) => prefix + '/' + k)
    const goodKeys = Object.entries(object)
        .filter(([k, v]) => reference[k] != undefined)
        .filter(
            ([k, v]) =>
                !(typeof v == 'string') &&
    ...
    return firstLayerIntrus.concat(
        goodKeys
            .map(([k, v]) =>
                findIntrus(prefix + '/' + k, object[k], reference[k]),
            )
...
export function isConsistent(key, val, schema) {
    ...
    if (
        ...
        return [
            ...
            val,
            schema.type,
            `Got '${typeof val}' while 'String' expected as part of enum.`,
        ]
    }
    ...
    if (
        ...
        schema.metadata.enum &&
        typeof val == 'string' &&
        !schema.metadata.enum.includes(val)
    ) {
        return [
            false,
            key,
            val,
            schema.type,
            `Got '${val}' while expected values from enum are: ${schema.metadata.enum}.`,
        ]
    }
    ...
    if (schema.type == 'String' && typeof val != 'string') {
        return [
            ...
            val,
            schema.type,
            `Got '${typeof val}' while 'String' expected.`,
        ]
    }
    ...
    if (
        ...
        return [
            ...
            val,
            'Integer',
            `Got '${val}' while 'integer' expected.`,
        ]
    }
    ...
    if (schema.type == 'Number' && typeof val != 'number') {
        return [
            ...
            val,
            schema.type,
            `Got '${typeof val}' while 'Number' expected.`,
        ]
    }
    ...
    if (schema.attributes && typeof val != 'object') {
        return [
            ...
            val,
            schema.type,
            `Got '${typeof val}' while '${schema.type}' expected.`,
        ]
    }
    ...
    if (
        ...
        return [
            ...
            val,
            schema.type,
            `Got undefined while a string or number was expected.`,
        ]
    }
 ...
 * @Hidden
 */
export function typeErrors(flattened): Array<{
    attributeName: string
    actualValue: string
    expectedType: string
    error: string
...
export function mergeConfiguration<T extends Object>(
    ...
    if (!newAttributes || Object.keys(newAttributes).length == 0) {
        return new ConsistentConfiguration<T>(
            persistentData,
            {},
            persistentData,
        )
    ...
    const mergedConfig = _.cloneDeep(persistentData)
    _.mergeWith(mergedConfig, newAttributes)

    const newConfigWithSchema = _.cloneDeep(newAttributes)
    newConfigWithSchema.__proto__ = Object.getPrototypeOf(persistentData)

    const flattenedMerged = flattenSchemaWithValue(mergedConfig)

    const intrus = findIntrus('', newConfigWithSchema, persistentData)
    const missings = findIntrus('', persistentData, mergedConfig)
    const errorsType = typeErrors(flattenedMerged)
    /*
    ...
    */
    if (errorsType.length + missings.length) {
        return new InconsistentConfiguration(
            persistentData,
            newAttributes,
            mergedConfig,
            intrus,
    ...
    const nonCodeIntrus = intrus.filter((intru: string) => {
        const key = intru.split('/')[1]
        const metadata =
            flattenedMerged[key] && flattenedMerged[key][0]
                ? flattenedMerged[key][0].metadata
                : undefined
        if (!metadata) {
            return true
        }
