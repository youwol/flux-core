...
export class Scene<Model> {
    ...
    add(input: Model | Array<Model>): Scene<Model> {
        ...
        const inCache = this.inCache
            .filter(
                (m) =>
                    inputs.find(
                        (input: Model) =>
                            this.idGetter(m) === this.idGetter(input),
                    ) == undefined,
            )
            .concat(...inputs)

        const lastModelById = inCache.reduce((acc, e) => {
            acc[this.idGetter(e)] = e
            return acc
        }, {})

        /* We need to remove from the scene the object with id that do not correspond to the last one */
        const modelsToRemove = this.inScene.filter(
            (model) => lastModelById[this.idGetter(model)] != model,
        )
        ...
        const modelsToAdd = Object.values(lastModelById).filter(
            (object: Model) => !this.inScene.includes(object),
        )
        ...
        const inScene = this.inScene
            .filter((m) => !modelsRemoved.includes(m))
            .concat(modelsAdded)
        return new Scene(
            this.idGetter,
            this.addFunction,
